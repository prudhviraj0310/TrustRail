...
def _drive_to_pending(client_rz, make_payload, **kw) -> dict:
    ...
    body = client_rz.post("/transactions", json={"intent_id": created["intent_id"]}).json()
...
def _captured_event(order_id, *, payment_id="pay_1", amount=CAPTURE_AMOUNT, currency="INR") -> bytes:
    return json.dumps(
        {
...
def _audit_actions(client_rz, transaction_id) -> list[str]:
    return [e["action"] for e in client_rz.get(f"/transactions/{transaction_id}/audit").json()["events"]]
...
def test_webhook_returns_503_under_mock_gateway(client):
    ...
    r = client.post("/webhooks/razorpay", content=b"{}", headers={"X-Razorpay-Signature": "x"})
...
def test_webhook_rejects_bad_signature(client_rz):
    ...
    r = client_rz.post("/webhooks/razorpay", content=body, headers={"X-Razorpay-Signature": "bad"})
    assert r.status_code == 400
...
def test_duplicate_delivery_is_idempotent(client_rz, make_payload):
    ...
    assert "PAYMENT_CONFIRM_DUPLICATE_IGNORED" in _audit_actions(client_rz, created["transaction_id"])
...
def test_amount_mismatch_is_refused(client_rz, make_payload):
    ...
    assert "WEBHOOK_AMOUNT_MISMATCH" in _audit_actions(client_rz, created["transaction_id"])
...
def test_payment_failed_does_not_terminalise(client_rz, make_payload):
    ...
    failed = json.dumps(
        {"event": "payment.failed",
         "payload": {"payment": {"entity": {"id": "pay_x", "order_id": order_id, "status": "failed"}}}}
    ).encode()
    r = client_rz.post("/webhooks/razorpay", content=failed, headers={"X-Razorpay-Signature": _sign(failed)})
    assert r.status_code == 200
